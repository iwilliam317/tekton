...
def index(c=''):
	...
	if (c!=''):
		resultados = [x for x in produtos if x.categoria == c]

		if (c=='novidades'):
			resultados = [x for x in produtos if x.novidades == 1]
	else:
		resultados = produtos

	contexto = {'categoria': c, 'produtos' : resultados, 'quantidade': len(resultados)}
